  ...
  <div class="content">
    <div>
      <h1>{{ __('app.class.create') }}</h1>
        <form action="{{ route('admin.class.store') }}" id="main_form" method="POST">
          @csrf
            ...
            <label class="label label--required mb-2" for="student_count"><span
                class="label__text">{{ __('app.class.students_count') }}</span>
              <input class="input" type="number"
                    name="student_count" id="student_count"
                    placeholder="{{ __('app.class.students_count') }}">
            </label>
