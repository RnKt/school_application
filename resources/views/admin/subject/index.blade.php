...
@section('title', __('app.subject.title.many'))


@section('content')
  <div class="content">
    <div class="heading-action mb-8">
      <h1 class="heading heading--2 d-inline-block">{{ __('app.subject.title.many') }}</h1>
      <a href="{{ route('admin.subject.create') }}"
         class="heading-action__button button button--small">{{ __('app.subject.create') }}</a>
    </div>
    <div class="content__wrapper">
      <form action="{{ route('admin.subject.delete') }}" method="POST" id="main_form">
        @csrf
            ...
            <tr class="table__row" data-id="{{ $subject->id }}">
              ...
              <td class="table__cell align-right">
               <div class="table__cell-content">
                  {{$subject_requirement->where('subject_id', '=', $subject->id)->count()}}
                 </div>
              </td>
            </tr>
          @endforeach
